...
def test_market_dispatch_mock_loader(tmp_path: Path, logger):
    """
    Shows how the Market assigns a MockLoader when the parquet file
    is missing.
    """
    ctx = Context(logger=logger, data_root=str(tmp_path))
...
def test_market_dispatch_polars(tmp_path: Path, logger):
    """
...
def test_market_dispatch_duckdb(tmp_path: Path, logger):
    """
